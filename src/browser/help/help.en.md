...
### Four tabs
- **List** — sortable asset table. `Used by`/`Uses` are direct degrees; the `∑` columns are transitive closures (blast radius / bundle); the `Bundle` column is the owning Asset Bundle (`main` = unbundled, `resources` = loaded by path). Click to select, double-click (or <kbd>Enter</kbd>) to set it as the centre; <kbd>↑</kbd> <kbd>↓</kbd> move between rows, <kbd>Ctrl</kbd>/<kbd>⌘</kbd>+<kbd>F</kbd> find in the list (rows are virtualized, so the browser's native find can't see off-screen rows).
