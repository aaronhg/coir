...
## Tool surface (13 built-in read + 14 write, plus plugin commands)
...
| `deps(asset, direction?, depth?, type?, limit?)` | Dependencies (who depends on it / what it depends on) + usage-site selectors. Default 1 hop (flat); `depth>1` returns the multi-hop dependency **tree** (same shape as `coir deps -o json --depth N`). |
...
| `verify(file?, all?)` | **Offline structural validation** (no live engine): `{__id__}` refs resolve, node↔child↔parent + component back-refs, null gaps, orphans, `__type__` resolvability → `{ valid, entries, errors[], warnings[] }`. Run after an `edit_*` (or before relying on a file) to catch corruption the engine would reject. `all:true` runs it over **every** prefab/scene (one project-wide gate). |
| `native_verify(file, port?)` | **LIVE** twin of `verify`: cross-checks the file against the RUNNING Cocos editor (the cocos-extension native-verify endpoint must be started for this project) — reimport + instantiate, then confirm every node/component coir parses is one the engine actually builds → `{ valid, file, nodes, components, engine, fails[] }`, or `{ error }` (code `no-endpoint` / `not-imported`) when no live editor is reachable. Shares the core with `coir native-verify`. |
| `roundtrip(file?)` | **Offline edit-engine audit** — the headless complement to `native-verify`: per prefab/scene, a byte round-trip (serializer fidelity, WARN) + an add-then-remove invertible probe (compaction/clone corruption, ERROR). No file = whole project. |
| `status` / `rescan` | Server status / force a rescan |

**Write (`edit_*`, all with `dryRun?`/`backup?`/`force?`/`verify?`/`diff?`/`reimport?`)**

`set` · `set_uuid` · **`set_ref`** (point a property at a node/component `{__id__}`; `into?` = a sub-path inside a nested instance's source prefab) · `swap_uuid` (`all?` project-wide) · `rename` · `set_active` · `set_layer` · `transform` (pos/scale/rot) · `set_parent` · `add_node` · `rm_node` · `add_component` · `rm_component` · **`edit_batch(file, ops[])`** — apply many ops **atomically** (load once → apply each → one write; any failure writes nothing). `ops` = `[{op, …params}]` in seam shape (`selector`/`value`/`parent`/`name`/`type`/`asset`); `swap-uuid` not allowed in a batch.

> Per-write flags: `verify:true` validates the result before committing (refuses on a structural error); `diff:true` returns a unified diff of the change in the result (great with `dryRun:true`); `reimport:true` asks the running Cocos editor (native-verify endpoint) to reimport the file after the write (returns `reimported:true/false`). `set`/`set_uuid`/`set_ref` only edit fields that already exist (a missing key is refused unless `force:true`, which then sets `needsReimport`); `set_ref` refuses a non-reference field, `set`/`set_uuid` attach a soft `warning` on a value-kind mismatch.
