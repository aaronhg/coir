...
| `verify(file)` | **Offline structural validation** (no live engine): `{__id__}` refs resolve, node↔child↔parent + component back-refs, null gaps, orphans, `__type__` resolvability → `{ valid, entries, errors[], warnings[] }`. Run after an `edit_*` (or before relying on a file) to catch corruption the engine would reject. (The CLI also has `coir native-verify <file>` — `verify`'s LIVE twin that cross-checks against the running editor — but it needs the editor endpoint, so it's **CLI-only**, not an MCP tool.) |
