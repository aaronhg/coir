...
### 11.27 Native Edit Verification (live Cocos editor) + the Minimal Editor-Op Set

`coir edit` is verified two ways today: `test/cli.test.js` (synthetic fixtures) proves coir is **self-consistent** (coir writes → coir reads back the same), which is necessary but **circular** for the real question — *does the Cocos engine accept and correctly load what coir wrote?* To answer that we drove the **real Cocos Creator 3.8.6** as ground truth via a third-party MCP bridge (`DaxianLee/cocos-mcp-server`, ~50 tools, in `NewProject_386/extensions/`, HTTP `:3000`, registered as the `cocos-creator` MCP). Full record in **`docs/edit-verification/`** (README method · PLAN matrix · RESULTS per-test log · COVERAGE audit · `_design_raw.json` 224-candidate superset).

**Result**: every `edit` op + every value flag + every documented guard exercised through the live engine — **80 PASS, 0 fail**. Two GIGO/validation gaps were found *and fixed* (both in the pure seam, so CLI + MCP inherit them; covered by new `test/cli.test.js` cases, npm test green):
- **FINDING-1** — `add-component` didn't validate the type. Fix (`src/edit/ops.js`): resolve via `typeToken(scan, type)` (the same resolver `set --json` uses) → reject unknown non-cc names (exit 1) **and write the resolved token**. This also fixed a *latent bug*: a project-script class name was written as the bare name (engine → MissingScript); it now writes the compressed uuid token, natively confirmed (`add-component … MyCoirComp` → engine builds a real `MyCoirComp`). A typo'd `cc.*` builtin (e.g. `cc.Nope`) still passes — unknowable offline without a bundled cc-component registry, consistent with `set --json`.
- **FINDING-2** — array `set`/`set-uuid` had no bounds check (an out-of-range index silently null-padded the array). Fix (`src/edit/editPrefab.js setDeep`): reject a gap (index > length, exit 2); replace/append still allowed.

**The forward-looking point — coir would only need to own a *handful* of editor operations.** The 50-tool third-party server was convenient, but the native-verification loop is load-bearing on just a few primitives. If coir ever grows its own native-verify capability (the natural home is the existing read-only `cocos-extension/`, which already runs coir-core in-process inside the editor — see §11.15), it would only need to implement:

1. **reimport an asset** — *the irreplaceable one.* coir writes a file on disk; the editor holds a cached **imported** copy, so it must re-read from disk. This is both the **validity gate** (a malformed file errors on reimport) and the proof coir alone cannot give (coir reading its own output is circular). **Hard ordering rule**: reimport BEFORE any open/instantiate — S01 read a stale scene until reimported, then PASS.
2. **instantiate a prefab (or open a scene) + read a node/component back** (`node_get_node_info` / `component_get_component_info` / `scene hierarchy`) — turns "the file loads" (native-load) into "the engine read *my* value" (native-readback). The FINDING-1 script-component fix was *only* provable here: instantiate → the component reports `RootNode<MyCoirComp>` + `__scriptAsset.uuid`, i.e. a real component, not MissingScript.
3. **copy / create / delete an asset + refresh + asset-db-ready** — the *safety* layer: isolated fresh-uuid fixtures under `assets/_coirtest/` let ~90 destructive edits run without touching real assets (a raw file copy would collide uuids), and `create .ts + refresh` is how a script-component add gets a real script to resolve against (NewProject_386 ships none).

Two third-party tools were *expected* to help but didn't, which is why **instantiate became the canonical prefab readback**: `prefab_get_prefab_info` returns only metadata (no node/component values) and `prefab_validate_prefab` is broken (calls a non-existent `asset-db read-asset` message). Also `component_get_components` is extremely verbose (~10 KB/call) — prefer `component_get_component_info` or `node_get_node_info` for transform/active. A coir-owned implementation could expose exactly these three primitives (reimport · instantiate+readback · fixture I/O) with terse, purpose-built payloads, and skip the other ~45 tools entirely.

### 11.28 Edit Round Two: `verify` · atomic `batch` · `tree --values` · `--diff` (+ the MCP/edit fixes)

The native-verification work (§11.27) surfaced what coir was *missing to be trustable offline* and turned up real bugs; this round closes both. All in the shared seam, so CLI + MCP inherit each. **Data-usage note** that motivated some of this: measured against the `cocos-creator` MCP, coir's reads are ~20–50× smaller per call (coir returns raw serialized fields; the editor returns a full inspector descriptor tree with i18n labels/defaults/types — a single `component_get_component_info` was **24.8 KB** vs coir `get` **506 B**). So the additions stay terse by design.

- **`verify` — offline structural validator** (`verifyDoc` in `editPrefab.js`; `verifyData`/`verifyText` in ops). Checks every `{__id__}` resolves to a typed in-range entry, node↔child↔parent + component→node back-refs, null gaps, orphan (unreferenced) entries, and `__type__` resolvability. Surfaces: `coir verify <file>` (+ `edit <file> verify`), MCP `verify`, and a `--verify` **write gate** (validate the result, refuse the commit on a structural error). This is the answer to the whole §11.27 "is coir's output engine-valid without opening the editor?" — most breakage is structural and knowable; only pure `cc.*` semantics still need the engine. Exits non-zero → CI-gateable.
- **Atomic `batch`** — `runEdit` was decomposed into `applyArrayOp` (one pure per-op mutation on the loaded array) + a thin `runEdit`; `runBatch` then applies N ops to ONE loaded doc and emits ONE write, so a multi-step refactor is all-or-nothing (any failure writes nothing). `coir edit <file> batch <ops.json>` (file or inline JSON) + MCP `edit_batch`. swap-uuid is excluded (it's a raw-text patch, not an array op). Cuts the N-round-trip + non-atomic problem hit during the §11.27 batches.
- **`tree --values` — deep read.** `treeData({values:true})` inlines each node's + component's raw serialized value, so structure AND values come back in one call (no per-node `get`). CLI `--values`, MCP `tree {values:true}`.
- **`--diff`** — a dependency-free LCS unified-diff (`src/edit/diff.js`) of any edit's planned write (writes now carry `oldText`); works with `--dry-run` as a preview. CLI `--diff` (text-mode only, never corrupts `-o json`); MCP edit `{diff:true}` returns the diff in the result.
- **The bugs found + fixed** (also §11.27 / the MCP verification): `add-component` now validates/resolves the type via `typeToken` (a project-script class name → its compressed token — previously a bare name → MissingScript; an unknown non-`cc.` name refused); `setDeep` bounds-checks an array index (an OOB index silently null-padded the array); the MCP `edit_set` `value` (the one untyped param) is parsed back from a host-stringified JSON string with a `raw:true` escape; and MCP errors normalized to a single `✗` (engine-seam errors were double-prefixing). Full record in **[docs/edit-verification/](docs/edit-verification/)** (`RESULTS*.md` / `COVERAGE*.md`).
...
### 11.30 Verify Round Three: project-wide `--all` · the read-only `--roundtrip` audit · two nested-prefab check rules · goto preview-canvas alignment

§11.29 gave a *live* verifier but it needs an open editor — useless in cloud CI. This round adds the **offline complements** plus the nested-prefab understanding that the next round's edits depend on.

- **`verify --all`** (`verifyAllData`) — the `verifyDoc` structural check (§11.28) over **every** prefab/scene, no target. One project-wide CI gate for structural health; an unloadable/non-array file is a failure here (verify is about file *validity*). Exits 1 if any file is broken. MCP `verify {all:true}`.
- **`verify --all --roundtrip`** (`auditRoundtripData`; alias `coir roundtrip`; per-file `verify <file> --roundtrip`) — the **headless complement to `native-verify`** that runs on real project data in CI. Two probes per file: (a) **byte round-trip** `serialize(parse(raw))===raw` (serializer/diff fidelity, WARN — coir's 2-space output vs the editor's); (b) **invertible-edit probe** (`probeInvertible`: add-then-remove a node through the REAL engine — exercising `addNode`'s template clone + `removeNode`'s global `__id__` compaction — require canonicalization back to the original, ERROR). Only probes otherwise-valid files (broken ones → `unprobed:pre-broken`, plain `verify`'s job); normalizes empty-vs-absent `_children`/`_components` (addNode's one benign side effect) so it never false-flags. Three real findings while being built: a discriminated-union JSDoc TS couldn't narrow (→ the file-wide `'error' in x` convention), a `cc.Prefab` lacking `data→root` (→ skip pre-broken), addNode initing `_children:[]` (→ the canon normalization).
- **Nested prefabs, understood** ([docs/NESTED-PREFABS.md](docs/NESTED-PREFABS.md), new). The three structures (`cc.PrefabInstance` boundary, `CCPropertyOverrideInfo` value override addressed by `cc.TargetInfo.localID`/`fileId`, `cc.TargetOverrideInfo` cross-boundary reference), auto-sync, why dragging a refNode bakes a subtree, and §10 the **`should_hide_in_hierarchy`** preview Canvas (cocos-cli `createShouldHideInHierarchyCanvasNode`, `LockedInEditor`, never serialized). `scripts/prefab-anatomy.js` is the reusable inspector (fileId/`__id__` resolved to named nodes).
- **Two check rules** from that understanding: **`no-deep-instance-override`** (only an instance ROOT's own properties may be overridden — a propertyOverride on a node *inside* a nested instance is forbidden; references exempt; `files`/`ignoreProps`/`allowProps`) and **`no-editor-preview-leak`** (flags a saved `should_hide_in_hierarchy` node — the preview Canvas accidentally committed, a known public-project footgun). Classifiers `findInstanceOverrides`/`findPreviewCanvasLeaks` in `editPrefab.js`; the host precomputes `ctx.instanceOverrides`/`ctx.previewLeaks` (the I/O inputs, gated by `needsX(rules)`), same pure-violations contract.
- **goto path alignment.** In prefab-edit mode the live tree is wrapped in a temp `Scene` (+ the preview Canvas for UI prefabs), but coir's file paths start at the prefab root. `logicalRoot` (`panels/goto.js`) descends past both wrappers to the prefab root — **only** in prefab-edit mode (a real scene's root is kept, since coir's scene paths include it), detected by the preview Canvas (UI) or by querying the open asset's type (3D). Footer UI: Selected/native-verify merged onto one line.

### 11.31 Edit Round Three: the nested-prefab edit boundary (P2/P3 `set-ref`) · field-existence + value-type guards · `--reimport`

The editor (§11.28) *refused* every nested-instance edit. This round opens the **legitimate** ones — exactly the operations the editor itself performs at an instance boundary — each **live-validated** against the 3789 endpoint (fixture-copy → modify on disk → reimport → readback → fixture-delete, never touching real files).

- **`set-ref <sel> <targetNode[:Type]>`** — point a property at a node/component, the `{__id__}` counterpart of `set-uuid`'s `{__uuid__}`. **P1** intra-file `{__id__}`. **P3a** (target baked/addressable in the file but inside a nested instance) → inline `{__id__}` + a `cc.TargetOverrideInfo` on the outer root's `PrefabInfo.targetOverrides`. **P3b** (`--into <sourceSubPath>`) → target lives only in the instance's **source prefab**: resolve its `fileId` from `PrefabInfo.asset.__uuid__` and write **only** the override (inline null, `needsReimport`). Live-validated that the engine resolves the reference from the override **with no baked branch at all** — the user's key correction ("不是用刪去的"): I re-ran the probe with the entire baked subtree *removed*, confirming the `localID` resolves against the instantiated instance. `setCrossRef` in `editPrefab.js`.
- **P2** — a node-property op (`set-pos`/`rename`/`set-active`/…) on an instance **ROOT** authors/updates a root `CCPropertyOverrideInfo` (`setRootOverride` + the `instanceWrite` classifier: root → override, deeper → refused, in lockstep with `no-deep-instance-override`). Validated: rename an instance root offline → the engine shows the new name. No baking — Creator fills the rest on reimport.
- **Field-existence guard** — `set`/`set-uuid`/`set-ref` into a **missing object property** is refused (a typo, or a field Creator hasn't written, instead of silently creating a dead field the engine drops). The answer to "how does coir know `refNode`'s type?" — *it doesn't*, so it only edits fields Creator already wrote (which carry the validated type). `--force` creates the field but the result is `needsReimport`. The unifying model: **validated territory (field exists) = offline-complete; unvalidated (force-created / cross-ref) = needs Creator reconcile** — the same axis as `needsReimport`.
- **Value-type sanity** — the field's *existing value* reveals its type. `set-ref` **refuses** a field whose current value isn't a reference (`null`/`{__id__}`); `set`/`set-uuid` attach a non-blocking **`warning`** on a kind mismatch (`valueKind`: null/string/number/boolean/asset/ref/`cc.*`/array/object). Honest scope: proves the field is *real*, not that the value's type is *right* — half the guarantee, cheaply.
- **`--reimport`** (CLI flag / MCP `reimport` param) — closes the `needsReimport` loop automatically: after a successful write, `connect()` to the native-verify endpoint and `/reimport` the file (refreshes the editor's library; `↻ reimported`), degrading gracefully (write still succeeds) when no matching endpoint is reachable. The CLI commit path became async to carry it (`applyResult` → `await maybeReimport`); MCP `commitResult` likewise.
...
- **Headless CLI** (`src/cli.js`, `bin` registered as `coir`, zero runtime dependencies): dependency queries `deps`/`uses`/`closure`/`find`/`info` + a project-level audit `analyze` (stats/unused/orphans/atlas/size, = the node-run.js report) (`--where` prints locations as a selector you can paste back into edit, `--type` does type pruning, `-o json` for structured output) + **in-place editing of prefabs/scenes** `edit` (`tree` (structure discovery) / `get` / `set` / `set-uuid` / `set-ref` (node/component refs, incl. P2/P3 across a nested-instance boundary) / `swap-uuid` / `rename` / `set-parent` / `add` / `rm-*` / atomic `batch` …; real-delete + index compaction, template-by-example, nested-instance guards, **existing-field + value-type guards**, atomic + mtime write guards, `--diff`/`--reimport`; design in `docs/EDITING.md`) + a layered **verification stack**: `verify` (offline structural, `--all` = whole-project CI gate), `verify --all --roundtrip` (offline read-only edit-engine audit), and `native-verify` (the LIVE twin against the running editor via the `cocos-extension/` endpoint). The read/write logic is extracted into a shared seam (`src/edit/ops.js` + `src/seam/query.js`), with the CLI and the **MCP server** (`coir mcp`, a hand-rolled zero-dependency JSON-RPC/stdio, typed tools: reads unprefixed / writes `edit_*`, namespaced as `coir__<tool>` in a host; see `docs/MCP.md`) sharing one source. **Plugins can further contribute commands** (`coir <name>`, and one carrying `inputSchema` automatically becomes an MCP tool; see §11.14), **asset right-click menus** (`assetMenus`, independent of commands; see §11.17), **and CI rule checkers** (`rules`, for `coir check`). The project directory is given via `-C <dir>` or defaults to the current directory. `npm test` runs `test/*.test.js` (synthetic projects, CI-safe, **196 cases** across CLI / MCP / rules / topohash / plugin / search-index, with dual 3.5.2/3.8.6 cross-version fixtures); `test/node-run.js` runs the full report against real projects as a regression.
- **Embedding / sharing**: the `#topo=<blob>` **URL-snapshot viewer** (a neighborhood subgraph packed into the hash → open the link to see the topology directly, **no File API needed, cross-browser**; `encodeTopo`/`decodeTopo` in `src/core/topohash.js`, auto-shrinking depth to fit 256KB); the `import('coir')` **embedding outlet** (`exports` → `src/index.js`); the **Cocos Creator 3.5–3.8 extension** (`cocos-extension/`: right-click an asset to see layered dependents/dependencies + a `Coir 跳轉` goto panel (live-scene node ↔ coir path, aligned past the prefab-edit preview Canvas) + the opt-in **native-verify endpoint** that backs `coir native-verify`/`--reimport` + open a topology snapshot + plugin asset menus (e.g. anim/skel showing animation duration), running coir-core in-process, deployed by `install.sh`).
