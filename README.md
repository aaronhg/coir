...
- **鍵盤／手勢**：`Tab` 切換分頁（`Delete` 反向）、`Esc` 清空類型篩選、`/` 或 `Ctrl/Cmd+P` 快速搜尋；拓撲內 `↑↓` 同欄、`←→`（或 Mac 兩指橫滑）跨欄並選「畫面垂直中央最近」者、`Enter` 設為新中心、`−` 上一動、`+` 下一動、`Ctrl/Cmd+C` 複製名稱、`Ctrl/Cmd+F` 在當前拓撲中找尋（見下）。
...
面板右下角是 **native-verify** 開關，顯示 listen 的 port 與 cocos 版本。啟動後（下圖 `:3789`），headless 的 **`coir native-verify <檔>`** 就能請這個 live 編輯器重載＋實例化檔案、交叉確認「引擎建得出 coir 解析到的每個 node／component」——抓得到離線 `verify` 看不到的引擎語意問題（被靜默丟棄的 `cc.*`、壞到無法 import 的檔等）。
