...
### 拓撲
- **雙向、固定 5 欄滑動視窗、永遠全展**的依賴樹：被依賴往左展、依賴往右展、層0 置中；視窗以選中項的「距中心位移」滑動；偵測循環顯示 `↻`。只列資源。欄頭以符號表示方向＋層號（`←層2` `←層1` ◆ `→層1` `→層2`），層0 欄染色。
...
- **虛擬化＋找尋**：拓撲與 `/` 清單都只繪製**可見列**（垂直虛擬化，hub 大圖／大量結果也不卡）。因為捲出畫面的節點不在 DOM、原生 `Ctrl+F` 找不到，拓撲內建 **`Ctrl/Cmd+F`** 在當前樹中找尋：直接搜 cell 資料、命中**琥珀色高亮**、`Enter`/`⇧Enter` 下／上一個、`Esc` 關閉（不改中心、只垂直捲到該節點）。
